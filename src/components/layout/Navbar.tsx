...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-40 border-b border-white/20 bg-foam" style={{background: 'var(--voyago-foam)', opacity: 1}}>
      <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center justify-center gap-3 h-full">
          ...
          >
            <Image src="/logo/logo-icon.png" alt="Voyago Icon" width={40} height={40} priority style={{ width: 40, height: 40 }} />
          </motion.div>
        ...
        <div className="flex items-center gap-6">
          ...
          <button aria-label="Cart" className="relative p-2 rounded-full hover:bg-sand-200 transition" onClick={() => setOpen(true)}>
            {/* Cart count badge */}
            {totalItems > 0 && (
              <span className="absolute -top-1 -right-1 bg-coral-400 text-white text-xs rounded-full min-w-[20px] h-5 flex items-center justify-center px-1.5 font-bold shadow">{totalItems}</span>
            )}
